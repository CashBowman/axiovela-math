...
In Write-up, choose **Annotate**, then click a sentence or drag across a passage to highlight it and leave feedback. **Comments** opens the list for editing, resolving, reopening or exporting notes. Select current notes and choose **Send feedback** to request a read-only assistant proposal. **Review proposed revision** shows the changes; only **Apply revision** replaces the selected draft. Notes retain their original source revision. See [the annotation workflow and limits](annotations.md).

With Annotate off, double-click a Markdown preview block to select its originating source line. In a newly rendered LaTeX PDF, double-click a passage to navigate using compiler-generated SyncTeX data. Navigation restores an expanded preview so the source is visible. This is approximate line-level navigation for the main manuscript; it cannot recover LaTeX from imported research PDFs. Render again after source or bibliography changes. See [the implementation and its limits](math-harness.md).
