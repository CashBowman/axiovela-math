...
try{
 ...
 await page.getByRole('button',{name:'Lean Certificates',exact:true}).click();assert.equal(await page.getByLabel('research message').inputValue(),'Shared draft before formalization');assert.equal(await page.getByLabel('research conversation',{exact:true}).inputValue(),c.id);
 ...
 await page.getByLabel('research message').fill('FIXTURE_LEAN_ARTIFACT');await page.getByRole('button',{name:'Send message',exact:true}).click();await page.getByRole('heading',{name:'Formal source saved',exact:true}).waitFor();await page.getByRole('heading',{name:'Addition of zero',exact:true}).waitFor();await page.getByRole('heading',{name:'preflight',exact:true}).waitFor();assert.equal(await page.locator('textarea.code').count(),0);assert.equal(await page.getByText('theorem fixture_add_zero',{exact:false}).count(),0);checks.push('real saved Lean files and readable target appear automatically without a code panel');
 ...
 await page.getByRole('button',{name:'Research',exact:true}).click();assert.equal(await page.getByLabel('research conversation',{exact:true}).inputValue(),c.id);await page.getByRole('button',{name:'Library',exact:true}).click();assert.equal(await page.getByLabel('research conversation',{exact:true}).inputValue(),c.id);await page.getByRole('button',{name:'Lean Certificates',exact:true}).click();
 ...
 await page.getByRole('button',{name:'Run Lean check'}).click();await page.getByRole('heading',{name:'tool unavailable',exact:true}).waitFor();checks.push('missing checker remains an explicit blocker, never a success');
