...
try{
 ...
 await app.evaluate(({app})=>{
  ...
  Updates.prototype.check=function(){this.privateDistribution=false;this.state.privateDistribution=false;this.keys=[pair.publicKey.export({type:'spki',format:'pem'})];this.fetcher=async(url,{signal})=>url.includes('api.github.com')?Response.json([{tag_name:'v'+version}]):url.endsWith('axiovela-math-update.json')?Response.json(envelope):!globalThis.updateFixture.slow?new Response(bytes):new Response(new ReadableStream({start(controller){controller.enqueue(bytes.subarray(0,4));signal.addEventListener('abort',()=>controller.error(signal.reason),{once:true});}}));return check.call(this);};
 });
 await app.evaluate(({Menu})=>Menu.getApplicationMenu().items.find(i=>i.label==='Help').submenu.items.find(i=>i.label==='Check for updates…').click());await page.getByRole('button',{name:'Check for updates',exact:true}).click();await page.getByRole('button',{name:'Download update',exact:true}).waitFor();assert.equal(await page.getByLabel('Release channel').inputValue(),'stable');
