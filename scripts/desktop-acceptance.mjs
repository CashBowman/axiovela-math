...
try{
 ...
 await page.locator('.historyCard').first().getByRole('button',{name:'Pin',exact:true}).click();await page.getByRole('button',{name:'Unpin',exact:true}).waitFor();
 await page.locator('.historyCard').first().getByRole('button',{name:'Archive',exact:true}).click();await page.getByText('No matching conversations.').waitFor();await page.getByLabel('History visibility').selectOption('archived');await page.getByText('★ Desktop history fixture',{exact:true}).click();await page.getByRole('dialog',{name:'Conversation history'}).waitFor({state:'hidden'});assert.equal(await page.getByLabel('research conversation',{exact:true}).inputValue(),historyId);checks.push('packaged History renames, pins, archives and reopens the same logical conversation');
