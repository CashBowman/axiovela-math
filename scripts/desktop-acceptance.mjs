...
try{
 ...
 await page.getByRole('button',{name:'LaTeX',exact:false}).first().click();await page.getByRole('button',{name:'Render PDF',exact:true}).click();await page.getByRole('region',{name:'Publication PDF'}).waitFor({timeout:120000});checks.push('bundled Tectonic renders a real publication PDF');
 await page.getByRole('button',{name:'Markdown',exact:false}).first().click();await page.getByLabel('Manuscript source').fill('A sentence to annotate.');await page.getByRole('button',{name:'Annotate',exact:true}).click();await page.locator('.paperPreview p').click();await page.getByLabel('Annotation feedback').fill('Clarify this sentence.');await page.getByRole('button',{name:'Add comment',exact:true}).click();await page.getByRole('button',{name:'Comment 1',exact:true}).click();await page.getByText('Clarify this sentence.',{exact:true}).waitFor();checks.push('packaged sentence annotation, contextual note and margin pin');
