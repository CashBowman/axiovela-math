...
export default function ConversationHistory({project, onClose, onChanged}) {
  const dialog=useRef(), [scope,setScope]=useState(project.id),[query,setQuery]=useState(''),[archived,setArchived]=useState(false),[offset,setOffset]=useState(0),[revision,setRevision]=useState(0),[result,setResult]=useState({conversations:[],warnings:[]}),[loading,setLoading]=useState(true),[error,setError]=useState(''),[editing,setEditing]=useState(''),[title,setTitle]=useState('');
  ...
  useEffect(()=>{
    ...
    const timer=setTimeout(async()=>{
      try {const value=await request('/api/conversation-history?'+new URLSearchParams({project:scope,q:query,archived:archived?'1':'0',offset:String(offset)}));if(!canceled)setResult(value);}
      catch(e){if(!canceled)setError(e.message);}
      finally{if(!canceled)setLoading(false);}
    },200);
    return()=>{canceled=true;clearTimeout(timer);};
  },[scope,query,archived,offset,revision]);
  ...
  return <dialog ref={dialog} className="conversationHistory" aria-label="Conversation history" onCancel={onClose}>
    ...
    <div className="historyFilters">
      <label>Scope<select aria-label="History scope" value={scope} onChange={e=>{setScope(e.target.value);setOffset(0);}}><option value={project.id}>Current project</option><option value="">All conversations</option></select></label>
