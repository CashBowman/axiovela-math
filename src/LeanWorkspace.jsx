...
export default function LeanWorkspace({project,resetKey,beforeSend,onArtifact}){
 ...
 <p>{data.setup.state==='ready'?`The compiler and project imports passed a small theorem test (${data.setup.toolchain}). Your research proof is checked separately below.`:data.setup.message||'Install the compiler and project libraries, then test setup. Requires internet and several GB for mathlib.'}</p>
 ...
 <p className="hint">Checks use saved source. Full access also allows the assistant to run checks.</p>
 {error&&<p className="inlineError" role="alert">{error}</p>}{data.stale&&current?.sourceHash&&<p role="status" className="inlineError">The formalization changed since this check. These results describe an earlier version.</p>}
 {current&&(data.sourceHash||current.status!=='not-configured')?<><h3 className="checkHeading">{current.status.replaceAll('-',' ')}</h3><p>{current.message}</p>{current.checks.map((c,i)=><div className="checkRow" key={i}>{['present','clear'].includes(c.status)?<CheckCircle2 size={18}/>:<AlertCircle size={18}/>}<div><strong>{c.label}</strong><p>{c.detail}</p></div><span className="badge">{c.status.replaceAll('-',' ')}</span></div>)}<p className="hint">Checked {new Date(current.checkedAt).toLocaleString()}. {current.sourceHash?'Source snapshot: '+current.sourceHash.slice(0,12):''}</p></>:null}
 <details className="checkHistory"><summary>Check history ({data.records.length})</summary>{data.records.map(r=><div className="historyRow" key={r.id}><span>{new Date(r.checkedAt).toLocaleString()}</span><span>{r.status.replaceAll('-',' ')}</span></div>)}</details>
 </div></Panel>;
