...
export function Panel({title,label,action,children,className='',expandable=false,expandLabel='reader',restoreKey}){
 ...
 useEffect(()=>{if(!expanded)return;const previous=document.activeElement;button.current?.focus();const escape=e=>{if(e.key==='Escape'){root.current?.dispatchEvent(new CustomEvent('math-panel-expand',{detail:false}));setExpanded(false);previous?.focus();}};window.addEventListener('keydown',escape);return()=>window.removeEventListener('keydown',escape);},[expanded]);
...
const emptySources = [];
export function Preview({source='',lean=false,hideLean=false,bibliography,imageUrl,sourceLocations=false,onSourceLine,sources=emptySources,onFollowup,prose=false}){
 const handlers=useRef();handlers.current={onSourceLine,onFollowup};
 return useMemo(()=>{
 ...
 const blocks=sourceLocations?Object.fromEntries(['p','h1','h2','h3','h4','li','blockquote','pre','table','div'].map(tag=>[tag,({node,...props})=>{const line=lineMap[(node?.position?.start.line||0)-1];return React.createElement(tag,{...props,...(line&&line<=maxLine?{'data-source-line':line,onDoubleClick:e=>{e.preventDefault();e.stopPropagation();handlers.current.onSourceLine?.(line);}}:{})});}])):{};
 ...
 if(props.href?.startsWith('axiovela-followup:')){const f=directives.followups[Number(props.href.split(':')[1])];return f&&onFollowup?<button className="followupChip" title="Add to your message, without sending" onClick={()=>handlers.current.onFollowup?.(f.prompt)}>{f.label}</button>:<span>{f?.label}</span>;}
 ...
 },[source,lean,hideLean,bibliography,imageUrl,sourceLocations,sources,!!onFollowup,prose]);
}
